...
FOMO_VOLUME_MULT = 2.5  # 2.5x la moyenne
FOMO_ACCELERATION_THRESHOLD = 0.005  # 0.5% de retour moyen récent
...
class BehavioralBiasEngine:
    ...
    def _detect_disposition_effect(
        ...
        high = np.max(prices[-min(20, len(prices)):])
        current = prices[-1]
    ...
    def _detect_fomo(
        ...
        acceleration = np.mean(recent_returns)
